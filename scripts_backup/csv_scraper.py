...
def extract_workflow_content(driver):
    """Extract description and instructions from workflow page."""
    try:
        # Get description from description elements
        desc_elements = driver.find_elements(By.CSS_SELECTOR, "div[class*=\"description\"]")
        description = ""
        ...
        # Get all paragraphs for instructions
        ...
        for p in paragraphs:
            ...
            if text and len(text) > 30:
                ...
                if not any(skip in text.lower() for skip in [
                    "n8n.io", "sign in", "created by", "last update", 
                    "categories", "share", "use for free", "imprint",
                    "legal", "privacy", "report a vulnerability",
                    "all rights reserved", "thank you to the n8n community"
                ]):
                    instruction_text.append(text)
        
        # Clean up and join instructions
        instructions = "\n\n".join(instruction_text) if instruction_text else ""
        
        return {
            "description": description,
            "instructions": instructions,
            "content_length": len(description) + len(instructions),
...
def scrape_workflow_from_url(driver, url, workflow_name, used_count):
    ...
    try:
        ...
        content_data = extract_workflow_content(driver)
